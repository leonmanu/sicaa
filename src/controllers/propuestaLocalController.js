...
const cursoLocalService = require('../services/propuestaLocalService');

const vincularCurso = async (req, res) => {
    try {
        const resultado = await cursoLocalService.vincularCurso(req.body, req.user);
        console.log('propuesta vinculado localmente con éxito:', req.body);
        req.flash('success', 'propuesta vinculado localmente con éxito.');
        return res.status(200).json({ 
            success: true, 
            message: 'propuesta vinculado correctamente' 
        });
    } catch (error) {
        console.error('Error al vincular el propuesta:', error.message);
        req.flash('error', 'Error al vincular el propuesta: ' + error.message);
...
//         res.render('pages/propuesta/cursoLocalList', { 
//             cursos, 
...
const getCursosPorCiieId = async (req, res) => {
    try {
        ...
        res.render('pages/propuesta/cursoListTodos', { //vista pendiente
            cursosLocales, 
            user: req.user,
...
const viewFormAltaPorCargoClaveCiieClave = async (req, res) => {
    try {
        ...
        res.render('pages/propuesta/cursoLocalForm', {
            cargo,         // Objeto único
            cursosLocales, // Array para el forEach
            user: req.user
        });
