...
class PropuestaLocalService {
    ...
    try {
        ...
        if (!Array.isArray(raw) || raw.length < 10) {
            throw new Error("El formato de datos del propuesta es inválido");
        }
        ...
        return await cursoLocalRepo.postPropuestaLocal(dataParaGuardar);
...
async getCursosPorCiieId(ciieId) {
    ...
    return cursos.map(propuesta => {
        const total = inscriptosDeEstosCursos.filter(ins => 
            ins.cursoId.toString() === propuesta._id.toString()
        ).length;
...
async getPorIdOfertaOficial(IdOfertaOficial) {
    ...
    //     } catch (error) {
    //         console.error('Error obteniendo propuesta por ID oficial:', error.message);
    //         throw error;
    //     }
    // }
...
module.exports = new PropuestaLocalService();
